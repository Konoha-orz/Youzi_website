...
function binl2b64(a) {
    for (var t = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", r = "", e = 0; e < 4 * a.length; e += 3) for (var i = (a[e >> 2] >> 8 * (e % 4) & 255) << 16 | (a[e + 1 >> 2] >> 8 * ((e + 1) % 4) & 255) << 8 | a[e + 2 >> 2] >> 8 * ((e + 2) % 4) & 255, s = 0; 4 > s; s++) r += 8 * e + 6 * s > 32 * a.length ? b64pad : t.charAt(i >> 6 * (3 - s) & 63);
    return r
}
...
var Gravatar = {
    ...
    attach_profiles: function (a, t) {
        setInterval(Gravatar.send_stats, 3e3), a = "undefined" == typeof a ? "body" : a, t && "string" == typeof t && jQuery(t).addClass("no-grav"), jQuery(a + ' img[src*="gravatar.com/avatar"]').not(".no-grav, .no-grav img").each(function () {
            if (hash = Gravatar.extract_hash(this), uniq = 0, jQuery("#grav-" + hash + "-" + uniq).length) for (; jQuery("#grav-" + hash + "-" + uniq).length;) uniq++;
    ...
    render_card: function (a, t) {
        ...
        if (null != i) {
            ...
            var v = .25 * o;
            jQuery("#" + t).removeClass("pos-right pos-left").addClass(f).css({top: d - v + "px", left: l + "px"});
            var u = o / 2;
        ...
        r.stop().css({opacity: 0}).show().animate({opacity: 1}, 150, "linear", function () {
    ...
    tilt_gravatar: function () {
        if (this.active_grav = jQuery("img#" + this.active_id), !jQuery("img#grav-clone-" + this.active_hash).length) {
            ...
            var a = this.active_grav.offset().top + parseInt(this.active_grav.css("padding-top"), 10),
                t = this.active_grav.offset().left + parseInt(this.active_grav.css("padding-left"), 10), r = {
            ...
            if (this.active_grav.hasClass("grav-hijack")) var e = '<a href="http://gravatar.com/' + this.active_hash + '" class="grav-clone-a" target="_blank"></a>'; else var e = this.active_grav.parents("a:first").clone(!0).empty();
    ...
    listen: function (t, r) {
        for (this.notify_stack || (this.notify_stack = {}), t = "g" + t, this.notify_stack[t] || (this.notify_stack[t] = []), a = 0; a < this.notify_stack[t].length; a++) if (r == this.notify_stack[t][a]) return;
    ...
    extract_hash: function (a) {
        if (hash = /gravatar.com\/avatar\/([0-9a-f]{32})/.exec(jQuery(a).attr("src")), null != hash && "object" == typeof hash && 2 == hash.length) hash = hash[1]; else {
    ...
    add_card_css: function () {
        if (!jQuery("#gravatar-card-css").length) {
            ...
            if (t ? (a = t.replace(/\/js\/gprofiles(?:\.dev)?\.js.*$/, ""), r = t.split("?")[1] || !1) : a = "//s.gravatar.com", !r) var e = new Date,
                i = new Date(e.getFullYear(), 0, 1), r = Math.ceil(((e - i) / 864e5 + i.getDay() + 1) / 7),
    ...
    data: {}, init: function (a) {
        ...
        if (-1 != a.indexOf(".")) {
            if (parts = a.split(/\./), GProfile.data[parts[0]]) {
                ...
                for (i = 0, s = GProfile.data[parts[0]].length; i < s; i++) if (GProfile.data[parts[0]][i].type && parts[1] == GProfile.data[parts[0]][i].type || GProfile.data[parts[0]][i].shortname && parts[1] == GProfile.data[parts[0]][i].shortname || GProfile.data[parts[0]][i].primary && "primary" == parts[1]) return GProfile.data[parts[0]][i]
            }
            return ""
        }
